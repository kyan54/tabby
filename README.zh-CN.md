...
这是上游 [Tabby](https://github.com/Eugeny/tabby) 的一个个人分支（fork）。我维护它纯粹是为了
自己日常使用方便：它跟随上游的版本更新，并在其基础上叠加了一些贴合我个人使用习惯的小修复和构建
调整。它不是官方构建，也不提供任何支持保证——如果上游的 Tabby 已经能满足你的需求，请优先使用上游版本。

本分支的预编译安装包发布在 [kyan54/tabby/releases](https://github.com/kyan54/tabby/releases)。

### v1.0.234-fix-v0.3

已签名并公证的 macOS 构建。CI 产出的 macOS `.dmg` / `.zip` 现在会使用 Developer ID 证书进行代码
签名，并通过 Apple 公证（notarization）。这样在 macOS 上不再出现“应用已损坏 / 无法验证开发者”的
提示，双击即可正常打开。本次发布未改动任何应用代码。Windows 与 Linux 的产物不受影响（Linux 无需
签名；Windows 签名暂未配置）。

### v1.0.234-fix-v0.2

进一步加固 v0.1 的修复。此前只要触发了一次意外的选区事件，残留的搜索高亮仍可能被重新判定成
“你自己的选区”——最典型的是粘贴后终端重绘的时候。这会导致 `Ctrl+C` 显示“已复制”而不是中断命令，
也会让右键把搜索文本复制进剪贴板（于是右键粘贴的是搜索内容，而且“只生效一次”）。

现在，只有由真正的**鼠标左键拖拽**产生的选区才会被算作“你自己的选区”。由搜索创建的选区，或者由
终端输出 / 滚动 / 重绘重新触发的选区，都不会再改变这一判定。因此 `Ctrl+C`、选中即复制
（copy-on-select）和右键（剪贴板模式）都会把搜索高亮当作“不是选区”——`Ctrl+C` 执行中断，右键执行
粘贴。

### v1.0.234-fix-v0.1

搜索高亮（即 `Ctrl+Shift+F` 留下的终端选区）是程序生成的，而不是你手动选的，所以它绝不应该混进
剪贴板。在 Windows 上，Tabby 默认 `rightClick: clipboard` 且 `copyOnSelect: true`，此前会让这种
高亮通过多条路径泄漏进剪贴板。本分支堵住了所有这些路径：

* **右键**不再复制残留的搜索高亮——它会像没有选区一样执行粘贴（或弹出菜单）。只有你自己产生的选区
  （例如鼠标拖拽）才会被复制。
* 当只存在搜索高亮时，**`Ctrl+C`** 现在会发送 `SIGINT`（中断正在运行的命令），而不是复制高亮的
  文本。真正的选区仍然像以前一样被复制。
* **关闭搜索面板**现在会清除匹配选区，因此不会再有残留内容之后被选中即复制、右键或 `Ctrl+C` 抓取。
